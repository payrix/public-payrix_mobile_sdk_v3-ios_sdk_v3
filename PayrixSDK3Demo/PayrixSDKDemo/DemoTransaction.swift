...
class DemoTransaction: UIViewController, PayrixSDKDelegate
{
	...
	func performTxn()
	{
		if !((self.currentTransaction.amount ?? 0) > 0){
			sharedUtils.showMessage(theController: self, theTitle: "Amount must be greater than 0", theMessage: "")
			return
		}
	...
	{
		if (txtCardHolder.text?.isEmpty ?? false) || txtCardHolder.text == ""
		{
			sharedUtils.showMessage(theController: self, theTitle: "Empty Name Field", theMessage: "Please provide your first name and/or last name.")
			return
		}
